...
void Camera::resize(int width, int height) {
	float half_fov = 0.5f * fov;

	float half_width  = 0.5f * width;
	float half_height = 0.5f * height;

	float d = half_width / tanf(half_fov);

	top_left_corner = Vector3(-half_width, half_height, d);
	u_axis = Vector3::normalize(Vector3(width,  0.0f,   0.0f));
	v_axis = Vector3::normalize(Vector3(0.0f,  -height, 0.0f));
}
